...
void AddDeduplicationInfoTransform::transform(Chunk & chunk)
{
    ...
    if (self_filter.removed_rows > 0)
    {
        LOG_DEBUG(getLogger("Deduplication::AddDeduplicationInfoTransform"),
            "Self deduplication removed {} rows out of {} rows, removed tokens {} out of {}, left rows {} in tokens {}, debug: {}",
            self_filter.removed_rows,
            chunk.getNumRows(),
            self_filter.removed_tokens,
            info->getCount(),
            self_filter.filtered_block->rows(),
            self_filter.deduplication_info->getCount(),
            info->debug());

        chassert(self_filter.filtered_block->rows() <= chunk.getNumRows());
    ...
    {
        LOG_TEST(getLogger("Deduplication::AddDeduplicationInfoTransform"),
            "No self deduplication performed for chunk with {} rows, debug: {}",
            chunk.getNumRows(),
            info->debug());
    }
}
