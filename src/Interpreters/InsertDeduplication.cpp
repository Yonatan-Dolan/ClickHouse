...
{
    LOG_TEST(logger, "filterSelfDeduplicate, debug: {}", debug());
    ...
    if (getCount() <= 1)
        return {original_block, shared_from_this()};
    ...
    {
        LOG_TEST(logger, "No duplicates found in self deduplication, debug: {}", debug());
        return {original_block, shared_from_this()};
    }
...
DeduplicationInfo::FilterResult DeduplicationInfo::filterOriginalBlock(const std::vector<std::string> & collisions, const String & partition_id)
{
    ...
    if (collisions.empty())
        return {.filtered_block = original_block, .deduplication_info = shared_from_this()};
...
DeduplicationInfo::FilterResult DeduplicationInfo::filterImpl(const std::set<size_t> & collision_offsets) const
{
    ...
    size_t remove_rows = 0;
    size_t remove_tokens = 0;
    ...
    for (size_t i = 0; i < offsets.size(); ++i)
    {
        ...
        {
            remove_rows += getTokenEnd(i) - getTokenBegin(i);
            remove_tokens += 1;
    ...
    chassert(remove_rows > 0);
    chassert(remove_rows <= block.rows());

    if (remove_rows == block.rows())
    {
        chassert(remove_tokens == getCount());
        ...
        return {
            ...
            .removed_tokens = remove_tokens
        };
    }

    auto cols = block.getColumns();
    for (auto & col : cols)
        col = col->filter(filer_column, block.rows() - remove_rows);
    ...
    const std::vector<std::string> & existing_block_ids, const std::string & partition_id, ContextPtr context)
{
    ...
    chassert(result.filtered_block->rows() + result.removed_rows == original_block->rows(), fmt::format(" {} + {} = {}",
        result.filtered_block->rows(), result.removed_rows, original_block->rows()));
    chassert(result.removed_rows > 0, "No rows were removed during deduplication");

    auto chunk = Chunk(result.filtered_block->getColumns(), result.filtered_block->rows());

    auto token_for_retry = result.deduplication_info->cloneSelf();
